...
export default function SongPage() { const { id = '' } = useParams(); const [song, setSong] = useState<DictionarySong | null>(null); const [videos, setVideos] = useState<Video[]>([]); const [loading, setLoading] = useState(true); const [pagination, setPagination] = useState({ page: 1, limit: 20, total: 0 });
  const load = async (page = 1) => { setLoading(true); try { const [s, v] = await Promise.all([dictionaryApi.getSong(id), dictionaryApi.getSongVideos(id, page, pagination.limit)]); setSong(s); setVideos(v.videos); setPagination(v.pagination); } finally { setLoading(false); } };
  useEffect(() => { void load(1); }, [id]); if (loading) return <Spin />; if (!song) return <Empty />;
  return <Card title={song.title}><Descriptions bordered column={1} items={[{ key: 'artist', label: 'Artist', children: song.artist }]} /><Table rowKey='id' style={{ marginTop: 16 }} columns={columns} dataSource={videos} locale={{ emptyText: <Empty description='No videos found' /> }} pagination={{ current: pagination.page, pageSize: pagination.limit, total: pagination.total, onChange: (page) => void load(page) }} /></Card>; }
