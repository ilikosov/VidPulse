...
export default function SongsListPage() {
  ...
  const columns: ColumnsType<DictionarySong> = useMemo(() => [
    { title: 'Title', dataIndex: 'title', render: (_v, row) => <Link to={`/songs/${row.id}`}>{row.title}</Link> },
  ...
  return <Space direction="vertical" size="middle" style={{ width: '100%' }}>
    <Typography.Title level={3} style={{ margin: 0 }}>Songs</Typography.Title>
    <Input.Search placeholder="Search by song title" value={searchDraft} onChange={(e) => setSearchDraft(e.target.value)} onSearch={() => updateParams({ q: searchDraft, page: 1 })} allowClear style={{ width: 260 }} />
    <Table rowKey="id" loading={loading} columns={columns} dataSource={items} locale={{ emptyText: <Empty description="No songs found" /> }} pagination={{ current: page, pageSize: limit, total: items.length, showSizeChanger: true, onChange: (nextPage, nextLimit) => updateParams({ page: nextPage, limit: nextLimit }) }} />
  </Space>;
}
