...
export default function DictionaryManagement() {
  ...
  const [editing, setEditing] = useState<any>(null); const [modalOpen, setModalOpen] = useState(false); const [tab, setTab] = useState('groups'); const [form] = Form.useForm(); const [groupType, setGroupType] = useState('all');
  ...
  return <>
    <Space style={{ marginBottom: 16 }}><Button type="primary" onClick={() => { setEditing(null); form.resetFields(); setModalOpen(true); }}>Add</Button><Upload showUploadList={false} beforeUpload={async (file) => { try { const r: any = await dictionaryApi.importFile(file as File); notification.success({ message: `Imported: ${r.inserted} inserted, ${r.updated} updated` }); await load(); } catch (e: any) { notification.error({ message: e.message }); } return false; }}><Button>Import from File</Button></Upload>{tab === 'groups' && <Select value={groupType} onChange={setGroupType} options={[{ value: 'all', label: 'All' }, { value: 'male', label: 'Male' }, { value: 'female', label: 'Female' }, { value: 'mixed', label: 'Mixed' }]} />}</Space>
