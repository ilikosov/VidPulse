...
export default function DictionaryManagement() {
  ...
  const load = async () => {
    setGroups(await dictionaryApi.getGroups());
    setArtists(await dictionaryApi.getArtists());
    setSongs(await dictionaryApi.getSongs());
    setEvents(await dictionaryApi.getEvents());
  };
  useEffect(() => {
    load().catch((e) => notification.error({ message: e.message }));
  }, []);
  const onSave = async () => {
    const v = await form.validateFields();
    const id = editing?.id;
    const map: any = {
      groups: [dictionaryApi.createGroup, dictionaryApi.updateGroup],
      artists: [dictionaryApi.createArtist, dictionaryApi.updateArtist],
      songs: [dictionaryApi.createSong, dictionaryApi.updateSong],
      events: [dictionaryApi.createEvent, dictionaryApi.updateEvent],
    };
    if (id) await map[tab][1](id, v);
    else await map[tab][0](v);
    setModalOpen(false);
    setEditing(null);
    form.resetFields();
    await load();
  ...
  const del = async (id: number) => {
    const map: any = {
      groups: dictionaryApi.deleteGroup,
      artists: dictionaryApi.deleteArtist,
      songs: dictionaryApi.deleteSong,
      events: dictionaryApi.deleteEvent,
    };
    await map[tab](id);
    await load();
    message.success('Deleted');
  };
  const rows = { groups, artists, songs, events }[tab].filter(
    (x: any) => tab !== 'groups' || groupType === 'all' || x.type === groupType,
  );
  const columns: any = {
    groups: [
      { title: 'Name', dataIndex: 'name' },
      { title: 'Type', dataIndex: 'type' },
      { title: 'Active', dataIndex: 'active', render: (v: any) => (v ? 'Yes' : 'No') },
      { title: 'Artists', dataIndex: 'artist_count' },
      {
        title: 'Actions',
        render: (_: any, r: any) => (
          <Space>
    ...
    artists: [
      { title: 'Name', dataIndex: 'name' },
      { title: 'Group ID', dataIndex: 'group_id' },
  ...
  return (
    <>
      <Space style={{ marginBottom: 16 }}>
        ...
        <Button onClick={() => setImportModalOpen(true)}>Import from File</Button>
        {tab === 'groups' && (
          <Select
            value={groupType}
          ...
          {tab === 'groups' && (
            <>
              ...
              <Form.Item name="type" label="Type" rules={[{ required: true }]}>
                <Select options={[{ value: 'male' }, { value: 'female' }, { value: 'mixed' }]} />
              </Form.Item>
