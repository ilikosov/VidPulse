...
> **Важно: песня — связь «многие-ко-многим».** В отличие от группы/артиста/события, у одного видео
> может быть **несколько** песен. Поэтому `videos.song_title` и `videos.song_id` — это лишь одиночный
> snapshot (обычно первая/основная песня), а **полный набор песен видео хранится в junction-таблице
> `video_songs`** (`video_id` ↔ `dictionary_songs.id`) — она и является source of truth для песен.
> При переходе на FK-модель резолвинг песен должен писать в `video_songs`, а не только в `videos.song_id`.
