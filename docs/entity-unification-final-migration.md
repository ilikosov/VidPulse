...
> **Примечание про песни (M:N):** у видео может быть **несколько** песен. Авторитетный набор песен
> хранится в junction-таблице **`video_songs`** (`video_id` ↔ `dictionary_songs.id`). Поля `videos.song_title`
> и `videos.song_id` — одиночные legacy/snapshot-значения (обычно первая/основная песня) и не отражают весь набор.
> Метрики ниже по `song_title`/`song_id` оценивают только этот одиночный snapshot, а не полноту `video_songs`.
