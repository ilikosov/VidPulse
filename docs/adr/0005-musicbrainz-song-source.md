...
- Полный прогон обогащения занимает минуты (сотни групп × пагинация) и через egress-прокси упирается
  в connect-timeout'ы на крупных группах. Митигируется **чанковым обогащением «по частям» с
  приоритетом по давности**: `MUSICBRAINZ_LIMIT` — размер чанка за рефреш (дефолт 150), а порядок
  задаёт колонка `dictionary_groups.songs_enriched_at` — каждый прогон берёт самые «старые»/ни разу не
  обогащённые группы (`null` → первыми) и проставляет им метку времени. Так нагрузка соединений за
  прогон ограничена, а упавшие по таймауту группы остаются без метки → идут первыми на следующем
  прогоне (импорт `merge` идемпотентен). За несколько прогонов набор покрывается полностью, далее цикл
  повторяется по самым старым. `MUSICBRAINZ_LIMIT=0` — весь набор за один прогон. БД — единственный
  источник правды о прогрессе (отдельный курсор не нужен).
