...
- Полный прогон обогащения занимает минуты (сотни групп × пагинация) и через egress-прокси упирается
  в connect-timeout'ы на крупных группах. Митигируется **чанковым возобновляемым обогащением «по
  частям»**: `MUSICBRAINZ_LIMIT` — размер чанка за рефреш (дефолт 150), сервер хранит курсор
  (`kpop_dict_mb_cursor`) и продвигает его каждым прогоном, зацикливая в конце. Так нагрузка соединений
  за прогон ограничена, а упавшие по таймауту группы добираются на следующем цикле (импорт `merge`
  идемпотентен). `MUSICBRAINZ_LIMIT=0` — весь набор за один прогон.
