...
Этот документ описывает модель данных VidPulse: каждую таблицу, её колонки, ключи и связи между сущностями. Схема задаётся миграциями Knex в [`packages/db/migrations/`](../packages/db/migrations) и является источником истины; данная страница — кураторское, удобочитаемое представление **текущей** схемы (после применения всех миграций).
...
Пользовательские подборки видео. Все видео в списке имеют один статус, и `status`
списка повторяет его (NULL, когда список пуст); операции применяются ко всему списку.
...
| `status`     | TEXT      | общий статус видео; NULL для пустого |
| `created_at` | TIMESTAMP | по умолчанию now                     |
| `updated_at` | TIMESTAMP | по умолчанию now                     |
