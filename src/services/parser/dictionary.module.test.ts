...
describe('DictionaryModule aliases normalization', () => {
  beforeEach(() => {
    ...
    vi.mocked(dictionaryService.getGroups).mockResolvedValue([
      { id: 1, name: 'LE SSERAFIM' },
      { id: 2, name: 'ITZY' },
    ]);
    vi.mocked(dictionaryService.getArtists).mockResolvedValue([
      { name: 'YUNJIN', group_name: 'LE SSERAFIM' },
      { name: 'YUNA', group_name: 'ITZY' },
    ] as any);
    vi.mocked(dictionaryService.getSongs).mockResolvedValue([{ title: 'DALLA DALLA' }] as any);
    vi.mocked(dictionaryService.getEvents).mockResolvedValue([{ name: 'INKIGAYO' }] as any);
