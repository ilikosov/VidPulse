...
function buildVideosListResponse(query) {
  ...
  return {
    items: ids.map((id) => fixture.videosList[id]).filter(Boolean),
  };
}
...
function registerMocks(baseUrl) {
  ...
  scope
    ...
    .query((query) => {
      ...
      if (
        query.id === fixture.channelId ||
        (Array.isArray(query.id) && query.id.includes(fixture.channelId))
      ) {
        return true;
      }
  ...
  scope.get('/youtube/v3/playlistItems').query(true).reply(200, fixture.playlistItemsList);
