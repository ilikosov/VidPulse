...
test('manual metadata editing from review queue updates status to new', async ({
  page,
  request,
}) => {
...
test('batch confirm download marks selected videos as downloaded', async ({ page, request }) => {
  ...
  const videosResponse = await request.get(
    'http://127.0.0.1:3000/api/videos?status=downloaded&limit=50',
  );
  const payload = await videosResponse.json();
  const downloadedTitles = payload.videos.map(
    (video: { original_title: string }) => video.original_title,
  );
