...
}>({ visibleGroupTypes: ['female', 'mixed'], setVisibleGroupTypes: async () => {} });

export function SettingsProvider({ children }: { children: React.ReactNode }) {
  const [visibleGroupTypes, setState] = useState<GroupType[]>(['female', 'mixed']);
  useEffect(() => {
    getSettings()
      .then((s) => setState((s.visible_group_types || 'female,mixed').split(',') as GroupType[]))
      .catch(() => undefined);
  }, []);
  ...
  return (
    <SettingsContext.Provider value={{ visibleGroupTypes, setVisibleGroupTypes }}>
      {children}
